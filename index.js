...
function filterStudentsByGrade(students, grade) {
  ...
  function getGradeExists(item) {
    let ii = Array();
    let res = item.hasOwnProperty("grade");
    let r;
    if (res) {
      r = item.grade === grade;
      if (r) {
        console.log(item.name + " має оцінку " + grade);
        return item;
      } else {
        return false;
      }
  ...
  let array = students.filter(getGradeExists);
}
...
function compareArrays(arr1, arr2) {
  ...
  let j1 = JSON.stringify(arr1);
  let j2 = JSON.stringify(arr2);
  // Використовуємо `console.assert()`, щоб вивести помилку, якщо масиви не однакові.
  console.assert(j1 === j2, "Assertion failed: Масиви не однакові!");
}
...
function validateUserInput(userInput) {
  ...
  console.assert(
    userInput.hasOwnProperty("username") === true,
  ...
  let len = userInput["password"].length;
  if (len < 8) {
...
function countVowelsAndConsonants(word) {
  ...
  const vowels = "aeiou"; // Голосні літери
